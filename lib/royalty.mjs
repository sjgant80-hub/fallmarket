...
export const SPLIT_POLICY = {
  ...
  guild: 0.10,       // 10% to the guild if the seller is guild-attested
 ...
 */
export function computeSplit(amount, { seller_did, ancestors_dids = [], guild_did = null, runner_did = null, is_guild_attested = false }) {
  const parts = [];
  ...
  } else if (is_guild_attested && guild_did) {
    // If no ancestors, reallocate ancestor pool to guild
    parts.push({
      did: guild_did,
      share_pct: (SPLIT_POLICY.ancestors + SPLIT_POLICY.guild) * 100,
      amount: round(amount * (SPLIT_POLICY.ancestors + SPLIT_POLICY.guild)),
      reason: 'guild (no ancestors)'
    });
  }

  if (is_guild_attested && guild_did && ancestors_dids.length) {
    parts.push({
      did: guild_did,
      share_pct: SPLIT_POLICY.guild * 100,
      amount: round(amount * SPLIT_POLICY.guild),
      reason: 'guild'
    });
  }
